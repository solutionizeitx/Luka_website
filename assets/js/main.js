...
/* Header entrance animation and sticky toggle when leaving hero */
(function($){
	"use strict";
	var $header = $('header.transparent_header');
	if (!$header.length) return;

	// show header with slide-down entrance
	$(window).on('load', function(){
		setTimeout(function(){
	...
	function checkSticky(){
		var $hero = $('#home');
		var heroBottom = $hero.length ? ($hero.offset().top + $hero.outerHeight()) : 0;
		var scrollTop = $(window).scrollTop();
		if (scrollTop >= Math.max(0, heroBottom - 20)) {
